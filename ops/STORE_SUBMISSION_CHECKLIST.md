...
- `apps/mobile/app.json` must keep the reviewed iOS bundle identifier, Android
  ...
  splash configuration, and the Face ID permission explanation.
- `apps/mobile/eas.json` must keep `preview` as the internal staging/acceptance
...
- No service-role key, database password, SMS-provider secret, Apple credential,
  Google Play credential, or EAS credential may appear in mobile source, public
  variables, store screenshots, or committed metadata.
...
after hosted staging has passed. On physical devices verify Arabic RTL and
English LTR, OTP delivery, onboarding, private profile controls, introductions,
mutual acceptance, conversations, activity, block/report flows, account
deletion, session restoration, biometric re-entry, sign-out, offline/recoverable
states, and app-switcher privacy behavior.
