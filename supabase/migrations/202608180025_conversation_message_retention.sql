create index if not exists introduction_conversations_retention_idx
  on private.introduction_conversations (closed_at, id)
  where status = 'closed'::public.conversation_status;

create table private.conversation_retention_runs (
  id uuid primary key default gen_random_uuid(),
  closed_before timestamptz not null,
  conversation_limit integer not null check (conversation_limit between 1 and 5000),
  conversations_selected integer not null check (conversations_selected >= 0),
  messages_deleted integer not null check (messages_deleted >= 0),
  recorded_at timestamptz not null default clock_timestamp()
);

revoke all on table private.conversation_retention_runs from public, anon, authenticated;
grant select, insert on table private.conversation_retention_runs to service_role;
