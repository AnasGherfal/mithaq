alter table private.conversation_messages
  add column if not exists client_nonce text;

alter table private.conversation_messages
  drop constraint if exists conversation_messages_client_nonce_check;

alter table private.conversation_messages
  add constraint conversation_messages_client_nonce_check
  check (
    client_nonce is null
    or (
      char_length(client_nonce) between 16 and 100
      and client_nonce ~ '^[A-Za-z0-9:_-]+$'
    )
  );

create unique index if not exists conversation_messages_sender_nonce_unique_idx
  on private.conversation_messages (conversation_id, sender_user_id, client_nonce)
