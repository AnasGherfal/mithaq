...
insert into private.controlled_introductions (
  ...
  'cooldown-test-stale'
) returning id into temporary table cooldown_ids;

select is(public.expire_controlled_introductions(100), 1, 'expiry worker transitions one stale offered introduction');
select is(
  (select status from private.controlled_introductions order by created_at limit 1),
  'expired'::public.introduction_status,
  'stale offer is persisted as expired'
);
select is(
  (select count(*)::integer from private.controlled_introduction_events where event_type = 'expired' and actor_reference = 'expiry-worker'),
  1,
