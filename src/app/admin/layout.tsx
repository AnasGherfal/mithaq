...
export default async function AdminLayout({
  ...
  const { data: accessData, error } = await supabase.rpc(
    "get_my_moderation_access",
    {},
  );
  if (error || !Array.isArray(accessData) || accessData.length === 0)
    notFound();
      ...
      <header className="border-b border-black/8 bg-[#102f29] text-white">
        <div className="mx-auto flex w-full max-w-7xl flex-wrap items-center justify-between gap-4 px-5 py-4 sm:px-8">
          ...
          <div className="flex items-center gap-2">
            <Link
              className="rounded-xl border border-white/15 px-3 py-2 text-xs font-black text-white/70 hover:bg-white/10"
              href="/"
            >
              الموقع العام
            </Link>
