...
export default async function AdminOverviewPage() {
  ...
  if (accessError || !Array.isArray(accessData) || accessData.length === 0)
    notFound();
  ...
  if (access.can_review) {
    ...
    if (!profileError)
      profileCount = Array.isArray(profiles)
        ? (profiles as QueueItem[]).length
        : 0;
    if (!photoError)
      photoCount = Array.isArray(photos) ? (photos as QueueItem[]).length : 0;
  }

  if (access.can_enforce) {
    const { data: reports, error } = await rpc.rpc("list_moderation_queue", {
      p_kind: "report",
      p_limit: 100,
    });
    if (!error)
      safetyCount = Array.isArray(reports)
        ? (reports as QueueItem[]).length
        : 0;
  }
  ...
  const queues: QueueCard[] = [
    {
      ...
      description:
        "راجع الطلب ثم أهّله. الدعوة هي القرار الذي يفتح إعداد العضوية للمستخدم.",
      count: waitlistCount,
    ...
    },
    {
      ...
      description:
        "اعتماد النبذة ومحتوى الملف أو طلب تعديل قبل أن يظهر العضو في الاستكشاف.",
      count: profileCount,
    ...
    },
    {
      ...
      title: "مراجعة الصور",
      description:
        "مراجعة الصور عبر روابط مؤقتة من التخزين الخاص بدون كشف مسار التخزين للمستخدمين.",
      count: photoCount,
      enabled: access.can_review,
    },
    {
      ...
      description:
        "فرز البلاغات والتحقيق فيها وإغلاقها ضمن سجل إشراف قابل للتدقيق.",
      count: safetyCount,
    ...
    <main className="px-5 py-8 sm:px-8 sm:py-10">
      <div className="mx-auto w-full max-w-7xl">
          ...
          <div className="flex flex-wrap items-end justify-between gap-6">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm leading-7 text-white/68">
                هذه مساحة تشغيل داخلية وليست جزءاً من تجربة الأعضاء. كل طابور له
                قرار واضح وصلاحيات مستقلة، ولا نعرض أرقام الهواتف أو بيانات
                تسجيل الدخول هنا.
              </p>
            </div>
            ...
            .map((queue) => (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-[11px] font-black text-[#9d702d]">
                      {queue.eyebrow}
                    </p>
                    <h2 className="mt-2 text-xl font-black text-[#153d35]">
                      {queue.title}
                    </h2>
                  </div>
                  {queue.count !== null ? (
                ...
                <div className="mt-5 text-xs font-black text-[#8b6228] group-hover:underline">
                  فتح الطابور ←
                </div>
              </Link>
            ))}
        ...
        <section className="mt-6 rounded-[1.75rem] border border-[#c99a52]/25 bg-[#c99a52]/8 p-5 sm:p-6">
          <h2 className="text-sm font-black text-[#8b6228]">
            كيف يعمل القبول؟
          </h2>
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            <div className="rounded-2xl bg-white/75 p-4">
              <div className="text-xs font-black text-[#153d35]">
                1 · قائمة الانتظار
              </div>
              <p className="mt-2 text-xs leading-6 text-black/52">
                مُرسل → مؤهل → <strong>مدعو</strong>. الدعوة تفتح إعداد العضوية.
              </p>
            </div>
            <div className="rounded-2xl bg-white/75 p-4">
              <div className="text-xs font-black text-[#153d35]">
              ...
              <p className="mt-2 text-xs leading-6 text-black/52">
                بعد الإعداد، يجب اعتماد الملف والصور حسب قواعد المراجعة قبل
                الاستكشاف.
              </p>
            </div>
            <div className="rounded-2xl bg-white/75 p-4">
              ...
              <p className="mt-2 text-xs leading-6 text-black/52">
                البلاغات منفصلة عن القبول وتذهب لمسار إشراف وتدقيق مستقل.
              </p>
            </div>
          </div>
        </section>
