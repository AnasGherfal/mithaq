...
import type { AdminWaitlistApplication, WaitlistStatus } from "@/lib/supabase/database.types";
...
function ApplicationCard({ application, currentFilter }: { application: AdminWaitlistApplication; currentFilter: string }) {
    ...
    <article className="rounded-3xl border border-black/8 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-3 font-mono text-[11px] font-bold tracking-wider text-black/35" dir="ltr">
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-3">
        <div className="rounded-2xl bg-[#f8f5ef] p-4">
          <div className="text-[11px] font-bold text-black/38">النوع والعمر</div>
        ...
        <div className="rounded-2xl bg-[#f8f5ef] p-4">
          ...
          <div className="mt-1 text-sm font-black text-[#153d35]">
            {application.current_city ?? "—"}{application.current_country_code ? ` · ${application.current_country_code}` : ""}
          </div>
          <div className="mt-1 text-[11px] text-black/40">{application.residency_type ? labels[application.residency_type] : ""}</div>
        </div>
        <div className="rounded-2xl bg-[#f8f5ef] p-4">
          <div className="text-[11px] font-bold text-black/38">الحالة الاجتماعية</div>
          <div className="mt-1 text-sm font-black text-[#153d35]">
...
export default async function AdminWaitlistPage({
  ...
  const currentFilter = filterStatuses.includes(requestedStatus as WaitlistStatus | "all") ? requestedStatus : "submitted";
  const statusFilter = currentFilter === "all" ? null : (currentFilter as WaitlistStatus);

  const [{ data: analyticsData, error: analyticsError }, { data: applications, error: applicationsError }] = await Promise.all([
    supabase.rpc("get_admin_waitlist_analytics", {}),
    supabase.rpc("list_admin_waitlist_applications", { p_status: statusFilter, p_limit: 100 }),
  ]);
  ...
  const submittedCount = currentFilter === "submitted" ? items.length : undefined;
  const summary = [
      ...
      <div className="mx-auto w-full max-w-7xl">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-black text-[#153d35]">مراجعة قائمة الانتظار</h1>
            <p className="mt-2 max-w-2xl text-sm leading-7 text-black/50">
              المسار الإداري واضح: <strong>مُرسل ← مؤهل ← مدعو</strong>. حالة «مدعو» هي الموافقة التي تسمح للمستخدم بالدخول إلى إعداد العضوية.
            </p>
          </div>
        ...
        <section className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {summary.map(([label, value]) => (
            <div className="rounded-3xl border border-black/8 bg-white p-5 shadow-sm" key={label}>
              <div className="text-xs font-bold text-black/42">{label}</div>
              <div className="mt-2 text-3xl font-black text-[#153d35]">{new Intl.NumberFormat("ar-LY").format(Number(value))}</div>
            </div>
          ))}
        </section>
        ...
        <nav className="mt-7 flex flex-wrap gap-2">
          {filterStatuses.map((status) => (
            <Link
              ...
              href={status === "submitted" ? "/admin/waitlist" : `/admin/waitlist?status=${status}`}
              key={status}
            >
        ...
        <div className="mt-5 flex items-center justify-between gap-4">
          <h2 className="text-lg font-black text-[#153d35]">{labels[currentFilter] ?? "كل الطلبات"}</h2>
          <span className="text-xs font-black text-black/42">
            {submittedCount !== undefined ? `${submittedCount} بانتظار المراجعة` : `${items.length} نتيجة`}
          </span>
        </div>

        <div className="mt-4 grid gap-4 xl:grid-cols-2">
          {items.length ? items.map((application) => (
            <ApplicationCard application={application} currentFilter={currentFilter} key={application.application_id} />
          )) : (
