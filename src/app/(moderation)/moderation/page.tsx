  ...
  return (
    <main className="min-h-svh px-4 py-5 md:px-6 md:py-7">
      <div className="mx-auto max-w-[1500px]">
        <header className="flex flex-col gap-4 rounded-3xl border border-black/10 bg-white px-5 py-5 shadow-sm md:flex-row md:items-center md:justify-between md:px-6">
          <div>
            ...
            <p className="mt-1 max-w-2xl text-sm leading-6 text-black/55">
              Review only what is required for safety and profile integrity. Every action is attributed and audited.
            </p>
          </div>
        ...
        {notice ? (
          <div className={`mt-4 rounded-2xl px-4 py-3 text-sm font-medium ${notice.tone === "ok" ? "bg-[#EAF5EF] text-[#17603E]" : "bg-[#FFF0F0] text-[#8D2424]"}`}>
            {notice.text}
          </div>
        ) : null}
          ...
          <div className="mt-5 grid gap-5 lg:grid-cols-[390px_minmax(0,1fr)]">
            <aside className="overflow-hidden rounded-3xl border border-black/10 bg-white shadow-sm">
              <div className="border-b border-black/10 p-4">
                <p className="text-xs font-bold tracking-[0.14em] text-black/40">QUEUE</p>
                <div className="mt-3 flex flex-wrap gap-2">
                  <FilterLink label="All" active={!filter} />
                  <FilterLink label="Reports" kind="report" active={filter === "report"} />
                  <FilterLink label="Photos" kind="photo" active={filter === "photo"} />
                  <FilterLink label="Profiles" kind="profile" active={filter === "profile"} />
                </div>
              </div>
              <div className="max-h-[calc(100svh-245px)] overflow-y-auto p-2">
                {queue.length === 0 ? (
                  <div className="p-6 text-center">
                    <p className="font-semibold text-[#12241F]">Queue is clear</p>
                    <p className="mt-1 text-sm leading-6 text-black/50">There are no open items in this view.</p>
                  </div>
                ) : (
                  queue.map((item) => (
                    <QueueCard
                      ...
                      item={item}
                      active={selectedKind === item.itemKind && selectedId === item.itemId}
                      filter={filter}
                    />
                  ))
                )}
            ...
            <section className="min-w-0 rounded-3xl border border-black/10 bg-white p-5 shadow-sm md:p-6">
              {selectedKind && selectedId && !selectedCase ? (
                <EmptyCase title="Case unavailable" body="It may have been resolved, removed, or is no longer available to your role." />
              ) : selectedCase ? (
                <CasePanel
                  record={selectedCase}
                  access={access}
...
function FilterLink({ label, kind, active }: { label: string; kind?: ModerationKind; active: boolean }) {
...
function QueueCard({ item, active, filter }: { item: QueueItem; active: boolean; filter: ModerationKind | null }) {
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.12em] ${kindTone(item.itemKind)}`}>
              {item.itemKind}
            </span>
            <span className="text-xs font-medium text-black/40">{humanState(item.state)}</span>
          </div>
          <p className="mt-2 truncate font-semibold text-[#12241F]">{item.displayLabel}</p>
          {item.category ? <p className="mt-1 text-xs text-[#9A6A24]">{humanState(item.category)}</p> : null}
        </div>
        <time className="shrink-0 text-[10px] text-black/35" dateTime={item.queuedAt}>
...
function CasePanel({ record, access, audit, photoUrl }: { record: CaseRecord; access: Access; audit: AuditItem[]; photoUrl: string | null }) {
  return (
    <div>
      <div className="flex flex-col gap-4 border-b border-black/10 pb-5 md:flex-row md:items-start md:justify-between">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <span className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.12em] ${kindTone(record.kind)}`}>{record.kind}</span>
            <span className="rounded-full bg-black/[0.05] px-2.5 py-1 text-xs font-semibold text-black/55">{humanState(record.state)}</span>
          </div>
          <h2 className="mt-3 text-2xl font-semibold tracking-tight text-[#12241F]">{caseTitle(record)}</h2>
          <p className="mt-1 break-all text-xs text-black/35">Case {record.itemId}</p>
        </div>
        <p className="rounded-2xl bg-[#F7F7F5] px-3 py-2 text-xs text-black/45">Target {shortId(record.targetUserId)}</p>
      </div>

      <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div className="min-w-0 space-y-5">
          {record.kind === "photo" ? <PhotoCase record={record} photoUrl={photoUrl} /> : null}
...
function ProfileCase({ record }: { record: CaseRecord }) {
  const fields: Array<[string, unknown]> = [
    ...
    ["Children", typeof record.hasChildren === "boolean" ? (record.hasChildren ? "Yes" : "No") : null],
  ];
  return <DetailCard title="Member-submitted profile">{fields.map(([label, entry]) => <DetailRow key={label} label={label} value={stringValue(entry) || "—"} />)}</DetailCard>;
}

function PhotoCase({ record, photoUrl }: { record: CaseRecord; photoUrl: string | null }) {
  return (
      ...
      <div className="overflow-hidden rounded-2xl border border-black/10 bg-[#F1F1EE]">
        {photoUrl ? (
          <div
            ...
            style={{ backgroundImage: `url(${JSON.stringify(photoUrl).slice(1, -1)})` }}
          />
        ) : (
          <div className="flex aspect-[4/5] items-center justify-center px-6 text-center text-sm leading-6 text-black/45">
            Photo preview is temporarily unavailable. Do not approve based on a missing preview.
          </div>
        )}
      </div>
      <div className="mt-3 grid gap-2 sm:grid-cols-2">
        <DetailRow label="Position" value={stringValue(record.position) || "—"} />
        <DetailRow label="Primary" value={record.isPrimary === true ? "Yes" : "No"} />
      </div>
      <p className="mt-3 text-xs leading-5 text-black/45">The preview link expires quickly and the raw storage path is never sent to this page.</p>
    </DetailCard>
  );
}
...
function ReportCase({ record }: { record: CaseRecord }) {
  return (
    <DetailCard title="Safety report">
      <DetailRow label="Category" value={humanState(stringValue(record.category)) || "—"} />
      <DetailRow label="Target" value={stringValue(record.targetDisplayName) || "Member"} />
      <DetailRow label="Reporter" value={stringValue(record.reporterDisplayName) || "Member"} />
      <div className="mt-4 rounded-2xl bg-[#FAFAF8] p-4">
        <p className="text-xs font-bold uppercase tracking-[0.12em] text-black/35">Member-provided report detail</p>
        <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-[#12241F]">{stringValue(record.details) || "No additional detail was provided."}</p>
      </div>
    </DetailCard>
  );
}

function CaseActions({ record, access }: { record: CaseRecord; access: Access }) {
    ...
    <DetailCard title="Case decision">
      <form action={moderateCaseAction} className="space-y-4">
        ...
        <label className="block text-sm font-semibold text-[#12241F]">
          Reason code
          <input name="reasonCode" maxLength={80} placeholder="e.g. profile_clear, unsafe_content" className="mt-2 w-full rounded-2xl border border-black/10 bg-[#FAFAF8] px-4 py-3 text-sm outline-none ring-[#0F4D3F] focus:ring-2" />
        </label>
        {record.kind !== "report" ? (
          <label className="block text-sm font-semibold text-[#12241F]">
            Review again after (optional)
            <input type="datetime-local" name="reviewAfter" className="mt-2 w-full rounded-2xl border border-black/10 bg-[#FAFAF8] px-4 py-3 text-sm outline-none ring-[#0F4D3F] focus:ring-2" />
          </label>
        ) : null}
        <div className="flex flex-wrap gap-2">
          {record.kind === "profile" ? (
            <>
              <DecisionButton intent="approve" label="Approve profile" tone="approve" />
              <DecisionButton intent="changes" label="Needs changes" />
              <DecisionButton intent="reject" label="Reject profile" tone="danger" />
            </>
          ) : null}
          {record.kind === "photo" ? (
            <>
              <DecisionButton intent="approve" label="Approve photo" tone="approve" />
              <DecisionButton intent="changes" label="Needs changes" />
              <DecisionButton intent="reject" label="Reject photo" tone="danger" />
            </>
          ) : null}
          {record.kind === "report" && access.canEnforce ? reportActions.map((action) => (
            <DecisionButton key={action} intent={action} label={humanState(action)} tone={action === "dismissed" || action === "closed" ? "danger" : undefined} />
          )) : null}
          {record.kind === "report" && !access.canEnforce ? (
            <p className="text-sm text-black/50">Reviewer role can inspect reports but cannot transition or enforce them.</p>
          ) : null}
        </div>
      </form>
    </DetailCard>
...
function EnforcementPanel({ record }: { record: CaseRecord }) {
  return (
    <DetailCard title="Member enforcement">
      <p className="text-sm leading-6 text-black/55">Restrict, suspend, or ban only when the case evidence supports it. Any active introduction and conversation involving this member closes immediately.</p>
      <form action={enforceMemberAction} className="mt-4 space-y-4">
        ...
        <label className="block text-sm font-semibold text-[#12241F]">
          ...
          <input required name="reasonCode" maxLength={80} placeholder="e.g. harassment_confirmed" className="mt-2 w-full rounded-2xl border border-black/10 bg-[#FAFAF8] px-4 py-3 text-sm outline-none ring-[#0F4D3F] focus:ring-2" />
        </label>
        <label className="block text-sm font-semibold text-[#12241F]">
...
function AuditPanel({ items }: { items: AuditItem[] }) {
  return (
    <div className="rounded-2xl border border-black/10 bg-[#FAFAF8] p-4">
      <p className="text-xs font-bold tracking-[0.14em] text-black/35">MITHAQ ACTION HISTORY</p>
      {items.length === 0 ? (
        <p className="mt-4 text-sm leading-6 text-black/45">No Mithaq moderator action has been recorded for this member yet.</p>
      ) : (
        <ol className="mt-4 space-y-3">
          {items.map((item) => (
            <li key={item.actionId} className="rounded-2xl border border-black/[0.07] bg-white p-3">
              <div className="flex items-start justify-between gap-2">
                <p className="text-sm font-semibold text-[#12241F]">{humanState(item.actionType)}</p>
                <time className="text-[10px] text-black/35" dateTime={item.recordedAt}>{formatShortDate(item.recordedAt)}</time>
              </div>
              <p className="mt-1 text-xs text-black/45">{item.actorRole} · {item.itemKind}</p>
              {item.reasonCode ? <p className="mt-2 break-words rounded-lg bg-black/[0.035] px-2 py-1.5 font-mono text-[10px] text-black/55">{item.reasonCode}</p> : null}
            </li>
          ))}
        </ol>
      )}
...
function DetailRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="grid gap-1 border-b border-black/[0.06] py-2.5 last:border-b-0 sm:grid-cols-[145px_1fr] sm:gap-4">
      ...
      <span className="whitespace-pre-wrap break-words text-sm leading-6 text-[#12241F]">{value}</span>
    </div>
  );
}

function DecisionButton({ intent, label, tone }: { intent: string; label: string; tone?: "approve" | "danger" }) {
  ...
  return <button type="submit" name="intent" value={intent} className={`rounded-xl px-3.5 py-2.5 text-sm font-semibold ${className}`}>{label}</button>;
}

function EnforcementButton({ intent, label, danger, restore }: { intent: string; label: string; danger?: boolean; restore?: boolean }) {
  const className = danger
    ? "bg-[#8D2424] text-white"
    : restore
      ? "border border-[#89B49F] bg-[#EEF5F2] text-[#0F4D3F]"
      : "border border-black/10 bg-white text-[#12241F]";
  return <button type="submit" name="intent" value={intent} formNoValidate={intent === "restore"} className={`rounded-xl px-3 py-2.5 text-sm font-semibold ${className}`}>{label}</button>;
}
...
function normalizeAccess(value: unknown): Access | null {
  ...
  const row = value[0];
  if (typeof row.moderation_role !== "string" || typeof row.can_review !== "boolean" || typeof row.can_enforce !== "boolean") return null;
  return { moderationRole: row.moderation_role, canReview: row.can_review, canEnforce: row.can_enforce };
}

function normalizeQueue(value: unknown): QueueItem[] {
  if (!Array.isArray(value)) return [];
  return value.flatMap((entry): QueueItem[] => {
    if (!isRecord(entry) || !isKind(entry.item_kind) || !isUuid(entry.item_id) || !isUuid(entry.target_user_id)) return [];
    return [{
      itemKind: entry.item_kind,
      itemId: entry.item_id,
      targetUserId: entry.target_user_id,
      reporterUserId: isUuid(entry.reporter_user_id) ? entry.reporter_user_id : null,
...
function normalizeCase(value: unknown): CaseRecord | null {
  if (!isRecord(value) || !isKind(value.kind) || !isUuid(value.itemId) || !isUuid(value.targetUserId)) return null;
  return { ...value, kind: value.kind, itemId: value.itemId, targetUserId: value.targetUserId, state: stringValue(value.state) };
}
...
function allowedReportActions(state: string) {
  ...
  if (state === "triaged") return ["investigating", "actioned", "dismissed", "closed"];
...
function caseTitle(record: CaseRecord) {
  if (record.kind === "profile") return stringValue(record.displayName) || "Profile review";
  if (record.kind === "photo") return `${stringValue(record.displayName) || "Member"} · photo review`;
  return `${stringValue(record.targetDisplayName) || "Member"} · safety report`;
}

function noticeText(value: string) {
  if (value === "saved") return { tone: "ok" as const, text: "Action saved and added to the moderation audit trail." };
  if (value === "forbidden") return { tone: "error" as const, text: "Your current staff role does not allow that action." };
  if (value === "invalid_input" || value === "invalid_action") return { tone: "error" as const, text: "That moderation action was not valid. Nothing was changed." };
  if (value === "action_failed") return { tone: "error" as const, text: "The action could not be completed. Nothing should be assumed from the UI; review the case state again." };
  return null;
}
...
function isKind(value: unknown): value is ModerationKind { return typeof value === "string" && kindValues.includes(value as ModerationKind); }
