...
export default async function WaitlistSuccessPage({ params }: PageProps) {
  ...
  const [{ data: application }, { data: referral }] = await Promise.all([
    supabase.from("waitlist_applications").select("status").eq("user_id", userId).single(),
    supabase.from("referral_codes").select("code").eq("owner_user_id", userId).eq("status", "active").maybeSingle(),
  ]);

  if (application?.status !== "submitted") redirect(`/${lang}/waitlist/questionnaire`);
  ...
  const copy = lang === "ar"
    ? {
        ...
        referralNote: "يمكنك مشاركة الرابط، لكنك لن ترى هوية الأشخاص الذين يسجلون من خلاله.",
    ...
    : {
        ...
        referralNote: "You can share this link, but you will not see the identities of people who register through it.",
    ...
    <main className="mx-auto w-full max-w-2xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      {referralPath ? (
        <section className="mt-8 rounded-3xl border border-primary/15 bg-primary/5 p-6">
          ...
          <code className="mt-3 block overflow-x-auto rounded-xl bg-background p-4 text-sm" dir="ltr">{referralPath}</code>
          <p className="mt-3 text-sm leading-6 text-muted-foreground">{copy.referralNote}</p>
        </section>
      ) : null}

      <a className="mt-8 inline-flex min-h-12 items-center rounded-xl bg-primary px-5 font-semibold text-primary-foreground" href={`/${lang}/waitlist/status`}>
        {copy.status}
      </a>
    </main>
  );
