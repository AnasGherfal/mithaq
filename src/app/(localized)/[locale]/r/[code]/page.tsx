import { ReferralCapture } from "@/features/waitlist/referral-capture";
...
export default async function ReferralLandingPage({ params }: PageProps) {
  ...
  const safeCode = /^[A-Z0-9]{8,16}$/.test(normalizedCode)
    ? normalizedCode
    : "";
  ...
  return (
    <main className="mx-auto w-full max-w-2xl px-4 py-16 sm:px-6 lg:px-8">
      {safeCode ? <ReferralCapture code={safeCode} /> : null}
      ...
      <p className="mt-6 rounded-2xl border border-primary/15 bg-primary/5 p-5 text-sm leading-7 text-muted-foreground">
        {copy.note}
      </p>
      <Link
