...
export function ScreenShell({ eyebrow, title, body, footer, bottomBar, rtl = false, scrollEnabled = true, brandVariant = "compact", children }: ScreenShellProps) {
  ...
  const content = (
    <View style={[styles.pageColumn, { alignItems: horizontalAlignment }]}>
      {brandVariant !== "none" ? (
        <View style={[styles.navRow, brandVariant === "full" ? styles.navRowFull : null, showSpaceSwitcher ? styles.navRowWithSpace : null, showSpaceSwitcher ? { flexDirection: rtl ? "row-reverse" : "row" } : { alignItems: brandVariant === "full" ? "center" : horizontalAlignment }]}> 
          <View style={{ alignSelf: brandVariant === "full" ? "center" : showSpaceSwitcher ? "auto" : rtl ? "flex-end" : "flex-start" }}>
            <BrandLogo rtl={rtl} variant={brandVariant} width={brandVariant === "full" ? Math.min(width - 60, 270) : undefined} />
          </View>
          {showSpaceSwitcher ? <ConnectionSpaceSwitcher locale={locale} fallbackSpace={fallbackSpace} /> : null}
        </View>
      ) : null}
      ...
      <View style={[styles.content, memberMode ? styles.contentMember : null, { alignItems: "stretch", alignSelf: "stretch" }]}>{children}</View>
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={["top", "left", "right"]}>
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        {scrollEnabled ? (
          <ScrollView style={styles.flex} contentContainerStyle={[styles.scrollContent, compact ? styles.scrollContentCompact : null, memberMode ? styles.scrollContentWithBar : null]} keyboardDismissMode={Platform.OS === "ios" ? "interactive" : "on-drag"} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>{content}</ScrollView>
        ) : (
          <View style={[styles.fixedContent, compact ? styles.scrollContentCompact : null, memberMode ? styles.fixedContentWithBar : null]}>{content}</View>
        )}
      </KeyboardAvoidingView>
...
const styles = StyleSheet.create({
  flex: { flex: 1 }, safeArea: { flex: 1, backgroundColor: colors.background }, pageColumn: { width: "100%", flex: 1 },
