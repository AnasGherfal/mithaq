...
export default function IntroductionsScreen() {
  ...
  const load = useCallback(async () => {
    ...
    if (first) {
      ...
      const row = await fetchPreview(first.introduction_id);
      setFeaturedPreviewId(first.introduction_id);
      setFeaturedPreview(row);
      setFeaturedLoading(false);
    } else {
      setFeaturedPreviewId(null);
  ...
  async function openIntroduction(item: IntroductionRow) {
    ...
    if (featuredPreviewId === item.introduction_id) {
      setPreview(featuredPreview);
  ...
  async function respond(accept: boolean) {
    ...
    setSelected((current) =>
      current
        ? { ...current, status: nextStatus, my_decision: nextDecision }
        : current,
    );
    setItems((current) =>
      current.map((item) =>
  ...
  if (selected) {
    const canRespond =
      selected.status === "offered" && selected.my_decision === "pending";
    const initials = preview?.display_name?.trim().charAt(0) || "م";
    const location = [preview?.city, preview?.country_code]
      .filter(Boolean)
      .join(" · ");
    ...
    return (
      <ScreenShell title={copy.detailTitle} rtl={rtl}>
        <View style={styles.detailPage}>
          ...
          {previewLoading ? (
            <View
              style={styles.loadingPortrait}
              accessibilityLabel={copy.loadingPreview}
            >
            ...
            <>
              ...
              >
                ...
                >
                  {[
                    preview.age_band_id
                      ? ageBands[preview.age_band_id - 1]
                      : null,
                    location || null,
                  ]
                    .filter(Boolean)
              ...
              <View style={styles.factGrid}>
                {preview.marital_status ? (
                  <Fact
                    label={copy.marital}
                    rtl={rtl}
                    value={copy.maritalStatus[preview.marital_status]}
                  />
                ) : null}
                {preview.has_children !== null ? (
                  <Fact
                    label={copy.children}
                    rtl={rtl}
                    value={preview.has_children ? copy.yes : copy.no}
                  />
                ) : null}
                {preview.origin_region ? (
                  <Fact label={copy.origin} rtl={rtl} value={preview.origin_region} />
                ) : null}
                {preview.occupation ? (
                  <Fact label={copy.occupation} rtl={rtl} value={preview.occupation} />
                ) : null}
                {preview.education ? (
                  <Fact label={copy.education} rtl={rtl} value={preview.education} />
                ) : null}
              </View>
            </>
          ) : (
              ...
              <PrimaryButton
                onPress={() =>
                  router.push({
                    ...
                    params: {
                      locale,
                      introductionId: selected.introduction_id,
                    },
                  })
                }
              >
    ...
    >
      {loading ? (
        ...
        >
          ...
          <Text
            style={[styles.emptyTitle, { textAlign, writingDirection }]}
          >
            {copy.emptyTitle}
          </Text>
          <Text style={[styles.emptyBody, { textAlign, writingDirection }]}> 
            {copy.emptyBody}
          </Text>
          <View style={styles.emptyAction}>
            <PrimaryButton
              tone="quiet"
              onPress={() =>
                router.push({ pathname: "/profile", params: { locale } })
              }
            >
              {copy.reviewProfile}
            </PrimaryButton>
        ...
        <View style={styles.listPage}>
          ...
          >
            {featuredLoading ? (
              <View style={[styles.featuredLoading, { height: portraitHeight }]}>
              ...
              <ProfilePortrait
                height={portraitHeight}
                initials={
                  featuredPreview?.display_name?.trim().charAt(0) || "م"
                }
                privacyLabel={copy.privatePhoto}
                rtl={rtl}
...
function selectFeatured(items: IntroductionRow[]) {
  return (
    items.find((item) =>
      ["offered", "mutually_accepted"].includes(item.status),
    ) ??
    items[0] ??
    null
  );
...
function HistoryRow({
  ...
  return (
    ...
    >
      ...
      <View style={styles.flex}>
        <Text
          style={[styles.historyName, { textAlign, writingDirection }]}
        >
          {copy.privateIntroduction}
        </Text>
        <Text
          style={[styles.historyDate, { textAlign, writingDirection }]}
        >
...
function introductionCopy(locale: MobileLocale) {
  if (locale === "ar") {
    return {
      title: "التعارف",
      body: "تعارف خاص ومدروس، وليس قائمة أشخاص للتصفح.",
      currentEyebrow: "التعارف الحالي",
      currentTitle: "شخص واحد يستحق التعرّف عليه بهدوء",
      privateIntroduction: "تعارف خاص",
      privateMember: "عضو ميثاق",
      privatePhoto: "صورة خاصة بالتعارف",
      openIntroduction: "مراجعة التعارف",
      detailTitle: "التعارف الخاص",
      backToList: "العودة إلى التعارفات",
      whyEyebrow: "لماذا ظهر هذا التعارف؟",
      whyTitle: "اختيار منضبط قبل أن يظهر لك الملف",
      reasonMutualPreferences: "فُحصت المتطلبات الأساسية في الاتجاهين قبل إنشاء التعارف.",
      reasonEligibility: "الحسابان مؤهلان للمشاركة وفق ضوابط الأمان والملف.",
      reasonPrivacy: "يبقى قرار كل طرف خاصاً حتى يصبح القبول متبادلاً.",
      about: "نبذة شخصية",
      decisionTitle: "هل ترغب في متابعة هذا التعارف؟",
      privateTitle: "خصوصية متبادلة",
      privateBody: "لا نعرض قرار الطرف الآخر قبل القبول المتبادل، ولا نكشف الهاتف أو أي بيانات خارج هذا التعارف.",
      safety: "الأمان أو الإبلاغ عن مشكلة",
      accept: "مهتم بالمتابعة",
      decline: "غير مناسب لي",
      acceptedTitle: "تم حفظ اهتمامك",
      acceptedBody: "لن نكشف قرار الطرف الآخر. سنخبرك فقط إذا أصبح القبول متبادلاً.",
      mutualTitle: "الاهتمام متبادل",
      mutualBody: "اختار الطرفان المتابعة. يمكنك الآن الانتقال إلى الخطوة التي تفتح التواصل الخاص.",
      continueAfterMutual: "فتح الخطوة التالية",
      emptyTitle: "لا يوجد تعارف حالياً",
      emptyBody: "هذا طبيعي. يبقى ملفك مؤهلاً، ويظهر التعارف فقط عندما يجتاز الطرفان المتطلبات الأساسية.",
      reviewProfile: "مراجعة ملفي وتفضيلاتي",
      history: "التعارفات السابقة",
      ...
      loadingPreview: "جارٍ تحميل التعارف الخاص",
  ...
  return {
    title: "Introductions",
    body: "A private, considered introduction—not a catalogue of people.",
    currentEyebrow: "CURRENT INTRODUCTION",
    currentTitle: "One person worth considering calmly",
    privateIntroduction: "Private introduction",
    privateMember: "Mithaq member",
    privatePhoto: "Introduction-only photo",
    openIntroduction: "Review introduction",
    detailTitle: "Private introduction",
    backToList: "Back to introductions",
    whyEyebrow: "WHY THIS APPEARED",
    whyTitle: "A controlled selection before a profile reaches you",
    reasonMutualPreferences:
      "Core requirements were checked in both directions before the introduction was created.",
    reasonEligibility:
      "Both accounts are eligible under profile and safety participation controls.",
    reasonPrivacy:
      "Each decision remains private unless interest becomes mutual.",
    about: "About",
    decisionTitle: "Would you like to continue this introduction?",
    privateTitle: "Mutual privacy",
    privateBody:
      "We do not reveal the other person’s decision before mutual interest, and phone numbers remain private.",
    safety: "Safety or report a concern",
    accept: "I’m interested",
    decline: "Not for me",
    acceptedTitle: "Your interest is saved",
    acceptedBody:
      "We will not reveal the other person’s decision. You will only be notified if interest becomes mutual.",
    mutualTitle: "Interest is mutual",
    mutualBody:
      "Both people chose to continue. You can now move to the protected communication handoff.",
    continueAfterMutual: "Open the next step",
    emptyTitle: "No introduction right now",
    emptyBody:
      "That is normal. Your profile remains eligible, and an introduction appears only when both people pass the core requirements.",
    reviewProfile: "Review my profile and preferences",
    history: "Previous introductions",
    ...
    loadingPreview: "Loading private introduction",
...
const styles = StyleSheet.create({
  ...
  historyList: { gap: 9, marginTop: 11 },
  historyRow: {
    width: "100%",
    minHeight: 72,
    alignItems: "center",
  ...
  reasonList: { width: "100%", gap: 10, marginTop: 14 },
  reasonRow: { width: "100%", alignItems: "flex-start", gap: 10 },
  reasonRowCompact: { gap: 8 },
  reasonText: {
    flex: 1,
    color: colors.muted,
    fontSize: 13,
  ...
  reasonTextCompact: { fontSize: 12, lineHeight: 19 },
  about: {
    width: "100%",
    color: colors.foreground,
    fontSize: 15,
  ...
  statusPillCompact: { paddingHorizontal: 9, paddingVertical: 7 },
  ...
  decisionSection: { width: "100%", gap: 10 },
  decisionTitle: {
    width: "100%",
    color: colors.foreground,
    fontSize: 17,
