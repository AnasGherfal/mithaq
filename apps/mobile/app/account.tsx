...
import { clearVisibleNotifications, unregisterCurrentPushDevice } from "@/lib/push-notifications";
...
export default function AccountScreen() {
  ...
  const copy = rtl
    ? {
        ...
        photos: "صوري الخاصة",
        photosBody: "الصور اختيارية. «خصوصية أولاً» يبقيها مخفية، والملف المفتوح قد يعرض صورة معتمدة باختيارك.",
        ...
        safety: "مركز السلامة",
        safetyBody: "البلاغات والحظر وضوابط حماية التعارف.",
        notifications: "الإشعارات",
        notificationsBody: "تنبيهات محايدة على شاشة القفل افتراضياً، وأنت تختار إن كان مسموحاً بإظهار تفاصيل أكثر.",
        security: "أمان الجهاز والحساب",
        securityBody: "القفل البيومتري والجلسات المسجلة على أجهزة أخرى.",
        development: "للتطوير فقط",
        familyPreview: "معاينة التسليم العائلي",
        familyPreviewBody: "جرّب مشاركة جهات الاتصال والصورة بعد إشراك العائلة بدون حساب ثانٍ أو أرقام حقيقية.",
        activityPreview: "معاينة رحلة النشاط",
        activityPreviewBody: "راجع تسلسل الاهتمام والتعارف والقبول والمحادثة ودائرة الثقة ببيانات محلية وهمية.",
        recognitionPreview: "معاينة معرفة الشخص",
        recognitionPreviewBody: "جرّب خيار «أعرف هذا الشخص» والإخفاء المتبادل بدون عضو حقيقي أو تغيير أي بيانات.",
        app: "التطبيق",
        language: "اللغة",
        languageBody: "العربية · اضغط للتبديل إلى English",
        signOut: "تسجيل الخروج",
        signOutBody: "إنهاء الجلسة المحفوظة على هذا الجهاز وإيقاف تنبيهاته الخاصة بالحساب.",
        loadErrorTitle: "تعذر تحميل حسابك",
        loadErrorBody: "تحقق من اتصالك ثم حاول مرة أخرى.",
        retry: "إعادة المحاولة",
        languageError: "تعذر حفظ اللغة الآن. حاول مرة أخرى.",
      }
    : {
        title: "Account",
        ...
        photosBody:
          "Photos are optional. Private first keeps them hidden; an Open profile may show an approved photo by your choice.",
        ...
        safetyBody: "Reports, blocks, and controls that protect introductions.",
        notifications: "Notifications",
        notificationsBody: "Neutral lock-screen updates by default. You decide whether more detail is allowed.",
        security: "Device & account security",
        securityBody: "Biometric protection and sessions on other devices.",
        development: "Development only",
        familyPreview: "Family handoff preview",
        familyPreviewBody:
          "Test contact sharing and the after-family photo stage without a second account or real phone numbers.",
        activityPreview: "Activity journey preview",
        activityPreviewBody:
          "Review interest, introduction, acceptance, conversation, and Trusted Circle stages with local fake data.",
        recognitionPreview: "Recognized-person preview",
        recognitionPreviewBody:
          "Test “I know this person” and the reciprocal privacy hide without a real member or database change.",
        app: "App",
        language: "Language",
        languageBody: "English · tap to switch to العربية",
        signOut: "Sign out",
        signOutBody: "End the saved session on this device and unregister its private account alerts.",
        loadErrorTitle: "We couldn’t load your account",
        loadErrorBody: "Check your connection, then try again.",
        retry: "Try again",
        languageError: "We couldn’t save the language right now. Try again.",
      };
  ...
  function open(pathname: AccountPath) {
    router.push({ pathname, params: { locale } });
  }
        ...
        <View style={styles.page}>
          ...
          <SettingsGroup title={copy.marriage} rtl={rtl}>
            ...
            <SettingsRow
              ...
              onPress={() => open("/profile-visibility")}
            />
            <SettingsRow
              rtl={rtl}
              icon="account"
              title={copy.trustedContacts}
              body={copy.trustedContactsBody}
              onPress={() => open("/trusted-contacts")}
              last
            />
          </SettingsGroup>
          ...
          <SettingsGroup title={copy.trust} rtl={rtl}>
            ...
            <SettingsRow
              ...
              onPress={() => open("/notification-privacy")}
            />
            <SettingsRow
              rtl={rtl}
              icon="privacy"
          ...
          {__DEV__ ? (
            <SettingsGroup title={copy.development} rtl={rtl}>
              <SettingsRow
                rtl={rtl}
                icon="activity"
                title={copy.activityPreview}
                body={copy.activityPreviewBody}
                onPress={() => open("/dev-activity")}
              />
              <SettingsRow
                rtl={rtl}
                icon="privacy"
                title={copy.recognitionPreview}
                body={copy.recognitionPreviewBody}
                onPress={() => open("/dev-recognition")}
              />
              <SettingsRow
                rtl={rtl}
                icon="account"
                title={copy.familyPreview}
                body={copy.familyPreviewBody}
                onPress={() => open("/dev-family-handoff")}
                last
              />
            </SettingsGroup>
          ) : null}
...
function SettingsGroup({ title, rtl, children }: { title: string; rtl: boolean; children: ReactNode }) {
  return (
    <View style={styles.groupWrap}>
      <Text style={[styles.groupTitle, { textAlign: rtl ? "right" : "left", writingDirection: rtl ? "rtl" : "ltr" }]}>
        {title}
      </Text>
  ...
  return (
    ...
    >
      ...
      <View style={[styles.rowCopy, { alignItems: rtl ? "flex-end" : "flex-start" }]}>
        <Text style={[styles.rowTitle, danger ? styles.rowTitleDanger : null, { textAlign, writingDirection }]}>
          {title}
        </Text>
...
const styles = StyleSheet.create({
  ...
  signOutGroup: {
    ...
    borderRadius: radius.lg,
    borderWidth: 1,
    borderColor: "rgba(180,35,63,0.16)",
    overflow: "hidden",
    backgroundColor: colors.surfaceRaised,
  },
});
