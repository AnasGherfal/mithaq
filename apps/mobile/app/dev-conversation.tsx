    ...
    >
      <View style={styles.stack}>
        ...
        <View style={styles.composerCard}>
          ...
          />
          <PrimaryButton disabled={!draft.trim()} onPress={() => setSent(true)}>{copy.send}</PrimaryButton>
          ...
          <View style={styles.controlsCard}>
            ...
            {confirmEnd ? (
              <View style={styles.confirmCard}>
                ...
                <PrimaryButton tone="quiet" onPress={() => setEnded(true)}>{copy.yesEnd}</PrimaryButton>
                <PrimaryButton tone="quiet" onPress={() => setConfirmEnd(false)}>{copy.cancel}</PrimaryButton>
              </View>
            ) : null}
          </View>
        )}
...
function previewCopy(locale: MobileLocale) {
  ...
  return {
    ...
    devBody: ar ? "Maya والرسائل هنا بيانات تجريبية وليست أعضاء أو رسائل حقيقية." : "Maya and these messages are sample data, not real members or real messages.",
    ...
    boundaryBody: ar ? "المحادثة تخص هذا التعارف فقط. درع العائلة أو الحظر أو إنهاء التعارف يوقف الوصول." : "The conversation belongs only to this introduction. Family Shield, blocking, or ending the introduction stops access.",
    emptyTitle: ar ? "ابدأ بشيء يساعد على فهم الزواج" : "Start with something that helps you understand marriage fit",
    emptyBody: ar ? "اختر سؤالاً كبداية أو اكتب بطريقتك. لا يتم الإرسال تلقائياً." : "Choose a starter or write your own. Nothing is sent automatically.",
    starters: ar
      ? [
    ...
    sampleReply: ar ? "بالنسبة لي الوضوح واحترام العائلة مهمان جداً." : "For me, clarity and respect for family are very important.",
    sampleMine: ar ? "أتفق، وأحب أن نكون واضحين من البداية." : "I agree, and I’d like us to be clear from the beginning.",
    ...
    controlsBody: ar ? "الإنهاء الهادئ مختلف عن الحظر أو الإبلاغ." : "Ending quietly is separate from blocking or reporting.",
    end: ar ? "تجربة إنهاء التعارف" : "Try ending introduction",
    confirmEnd: ar ? "راجع قرار الإنهاء" : "Review ending choice",
    confirmBody: ar ? "سيُغلق مسار الرسائل في التطبيق الحقيقي، لكنه لا يحظر الطرف الآخر." : "In the real app this closes the message path, but it does not block the other person.",
    yesEnd: ar ? "محاكاة الإنهاء" : "Simulate ending",
    cancel: ar ? "إلغاء" : "Cancel",
    endedTitle: ar ? "تم إنهاء التعارف في المعاينة" : "Preview introduction ended",
    endedBody: ar ? "هذه معاينة محلية فقط؛ لم يتغير أي حساب حقيقي." : "This was local only; no real account or introduction changed.",
  };
}
...
const styles = StyleSheet.create({
  ...
  partnerCard: { borderRadius: radius.xl, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surfaceRaised, overflow: "hidden", ...shadows.card },
  partnerContent: { padding: 15 },
  stagePill: { borderRadius: radius.pill, backgroundColor: colors.primaryWash, paddingHorizontal: 9, paddingVertical: 6 },
  ...
  boundaryCard: { borderRadius: radius.lg, borderWidth: 1, borderColor: colors.primarySoft, backgroundColor: colors.primaryWash, padding: 14 },
  ...
  emptyCard: { borderRadius: radius.xl, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surfaceRaised, padding: 15, alignItems: "center", ...shadows.card },
  ...
  starter: { borderRadius: radius.md, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surfaceMuted, padding: 11 },
  starterText: { color: colors.foreground, fontSize: 11, lineHeight: 18, fontWeight: "700" },
  messagesCard: { borderRadius: radius.xl, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surfaceRaised, padding: 14, gap: 8, ...shadows.card },
  ...
  theirBubble: { maxWidth: "84%", borderRadius: radius.lg, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surfaceMuted, padding: 11 },
  ...
  composerCard: { borderRadius: radius.xl, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surfaceRaised, padding: 14, gap: 9, ...shadows.card },
  composerTitle: { color: colors.foreground, fontSize: 13, fontWeight: "900" },
  input: { minHeight: 96, borderRadius: radius.md, borderWidth: 1, borderColor: colors.borderStrong, backgroundColor: colors.surfaceMuted, color: colors.foreground, padding: 12, fontSize: 13, lineHeight: 20, textAlignVertical: "top" },
  presenceNote: { color: colors.mutedSoft, fontSize: 9, lineHeight: 15 },
  controlsCard: { borderRadius: radius.lg, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surfaceRaised, padding: 14, gap: 7 },
