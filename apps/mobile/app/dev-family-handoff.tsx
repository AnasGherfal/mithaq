      ...
      <View style={styles.stack}>
        ...
        {otherShared ? (
          ...
          <View style={styles.waitingCard}>
            ...
            <PrimaryButton tone="quiet" onPress={() => setOtherShared(true)}>
              {copy.simulateTheirShare}
            </PrimaryButton>
          </View>
        )}
        ...
        {!myShared ? (
          <View style={styles.shareCard}>
            ...
            {confirmShare ? (
              <View style={styles.confirmCard}>
                ...
                <Text style={[styles.confirmBody, { textAlign: rtl ? "right" : "left" }]}>
                  {copy.confirmBody(chosen.name, chosen.phone)}
                </Text>
            ...
            {confirmShare ? (
              <PrimaryButton tone="quiet" onPress={() => setConfirmShare(false)}>
                {copy.cancel}
              </PrimaryButton>
            ) : null}
          </View>
...
function familyCopy(locale: MobileLocale) {
  ...
  return {
    ...
    body: ar
      ? "معاينة محلية فقط لا تحفظ أرقاماً ولا تراسل أحداً."
      : "A local-only preview that saves no numbers and contacts nobody.",
    back: ar ? "رجوع" : "Back",
    devTitle: ar ? "للتطوير فقط" : "Development only",
    devBody: ar
      ? "الأسماء والأرقام أدناه تجريبية ولا تمثل أعضاء أو جهات اتصال حقيقية."
      : "The names and numbers below are samples and do not represent real members or contacts.",
    ...
    photoPrivateBody: ar
      ? "محاكاة مشاركة الطرف الآخر وحدها لا تكشف صورتك. يجب أن تبدأ أنت مرحلة العائلة لصورتك."
      : "The other member sharing first does not reveal your photo. You must start the family stage for your own photo.",
    photoOpened: ar ? "بدأت مرحلة العائلة لصورتك" : "Your family stage has started",
    photoOpenedBody: ar
      ? "بعد مشاركة جهة اتصالك أنت، يمكن لصورتك اتباع خيار «بعد إشراك العائلة» لهذا التعارف."
      : "After you share your own trusted contact, your photo can follow the “after family involvement” setting for this introduction.",
    theirKicker: ar ? "جهة اتصال الطرف الآخر" : "OTHER MEMBER’S TRUSTED CONTACT",
    theirNote: ar
      ? "هذه البيانات تظهر فقط لأن الطرف الآخر اختار مشاركتها في التعارف."
      : "These details appear only because the other member chose to share them in the introduction.",
    theirWaiting: ar ? "الطرف الآخر لم يشارك جهة اتصال بعد" : "The other member has not shared a contact yet",
    theirWaitingBody: ar
      ? "لا يلزم أن يشارك الطرفان في نفس الوقت."
      : "Both people do not have to share at the same time.",
    ...
    chooseBody: ar
      ? "الاختيار هنا لا يرسل شيئاً حتى تؤكد المشاركة مرتين."
      : "Selecting a contact sends nothing until you explicitly confirm the share.",
    confirmTitle: ar ? "تأكيد المشاركة" : "Confirm sharing",
    confirmBody: (name: string, phone: string) =>
      ar
        ? `سيتمكن الطرف الآخر من رؤية ${name} ورقم ${phone}. ميثاق لن يراسله.`
        : `The other member will be able to see ${name} and ${phone}. Mithaq will not message them.`,
    noUndo: ar
      ? "بعد أن يرى الطرف الآخر الرقم لا يمكن جعله كأنه لم يُرَ، لذلك لا يوجد زر تراجع مضلل."
      : "Once the other member has seen the number, it cannot be made unseen, so there is no misleading undo button.",
    ...
    sharedBody: (name: string) =>
      ar
        ? `يمكن للطرف الآخر الآن رؤية بيانات ${name} في سيناريو الاختبار.`
        : `The other member can now see ${name}’s details in this test scenario.`,
    noAutomaticTitle: ar ? "ميثاق لا يتواصل مع العائلة تلقائياً" : "Mithaq does not contact family automatically",
    noAutomaticBody: ar
      ? "النسخة الأولى تشارك جهة الاتصال بين العضوين فقط. لا SMS ولا مكالمة ولا دعوة تُرسل للطرف الثالث."
      : "V1 shares the contact between the two members only. No SMS, call, or invitation is sent to the third party.",
...
const styles = StyleSheet.create({
  ...
  devCard: { borderRadius: radius.lg, backgroundColor: colors.goldSoft, padding: 14 },
  devTitle: { color: colors.gold, fontSize: 12, fontWeight: "900" },
  devBody: { color: colors.muted, fontSize: 10, lineHeight: 17, marginTop: 3 },
  photoCard: {
  ...
  sectionTitle: { color: colors.foreground, fontSize: 14, lineHeight: 22, fontWeight: "900", marginTop: 4 },
  sectionBody: { color: colors.muted, fontSize: 10, lineHeight: 18, marginTop: 3 },
  otherCard: {
  ...
  waitingCard: { borderRadius: radius.lg, backgroundColor: colors.surfaceMuted, padding: 14, gap: 8 },
  shareCard: {
    borderRadius: radius.xl,
    borderWidth: 1,
  ...
  successTitle: { color: colors.primaryStrong, fontSize: 14, fontWeight: "900" },
  contactList: { gap: 7 },
  contactCard: {
  ...
  contactCardSelected: { borderColor: colors.primary, backgroundColor: colors.primaryWash },
  contactRow: { alignItems: "center", gap: 10 },
  radio: {
    width: 20,
    height: 20,
    borderRadius: 10,
  ...
  contactName: { color: colors.foreground, fontSize: 14, fontWeight: "900" },
  contactMeta: { color: colors.muted, fontSize: 10, lineHeight: 16, marginTop: 2 },
  phone: {
  ...
  smallNote: { color: colors.muted, fontSize: 9, lineHeight: 16, marginTop: 6 },
  ...
  ruleCard: { borderRadius: radius.lg, backgroundColor: colors.surfaceMuted, padding: 14 },
});
