    ...
    <ScreenShell
      ...
      footer={
        <View style={styles.footer}>
          ...
          <Pressable
            accessibilityRole="button"
            onPress={() => router.push({ pathname: "/friendship-requests", params: { locale } })}
            style={({ pressed }) => [styles.footerLink, pressed ? styles.pressed : null]}
          >
          ...
          <View style={styles.list}>
            {connections.map((connection) => (
              <View key={connection.connectionId} style={styles.card}>
                <View style={[styles.identityRow, { flexDirection: rtl ? "row-reverse" : "row" }]}>
                  <View style={styles.avatar}>
                    <Text style={styles.avatarText}>
                      {connection.displayName.trim().charAt(0).toUpperCase()}
                    </Text>
                  </View>
                ...
                <PrimaryButton disabled tone="quiet">
                  {copy.chatSoon}
                </PrimaryButton>
              </View>
            ))}
...
function connectionsCopy(locale: MobileLocale) {
  if (locale === "ar") {
    return {
      ...
      body: "هذه قائمة الصداقات التي تم قبولها فعلياً. لا تظهر هنا أي علاقة من مساحة الزواج.",
      home: "الأصدقاء",
      requests: "الطلبات",
      summary: "اتصالات أصدقاء مقبولة",
      connected: "اتصال أصدقاء",
      chatSoon: "المحادثة قريباً",
      ...
      previewBody: "طبّق ترحيل اتصالات الأصدقاء على Supabase المرحلي لتفعيل هذه القائمة.",
  ...
  return {
    ...
    body: "These are accepted friendships only. Nothing from Marriage can appear in this list.",
    home: "Friends home",
    requests: "Requests",
    summary: "accepted friend connections",
    connected: "Friends connection",
    chatSoon: "Chat coming next",
    ...
    previewBody: "Deploy the Friends connection migration to hosted staging to activate this list.",
...
const styles = StyleSheet.create({
  ...
  summaryValue: { color: colors.primaryStrong, fontSize: 28, lineHeight: 34, fontWeight: "900" },
  summaryLabel: { width: "100%", color: colors.primary, fontSize: 12, lineHeight: 19, fontWeight: "800" },
