          ...
          {introductionMode ? (
            handoff ? (
              <View style={styles.handoffCard}>
                <Text style={[styles.handoffSectionTitle, { textAlign: rtl ? "right" : "left" }]}>
                  {copy.handoffSectionTitle}
                </Text>
                <Text style={[styles.handoffSectionBody, { textAlign: rtl ? "right" : "left" }]}>
                  {copy.handoffSectionBody}
                </Text>
                ...
                {handoff.otherShared && handoff.otherContactName && handoff.otherPhoneE164 ? (
                  <View style={styles.sharedContactCard}>
                    ...
                    <Text style={[styles.sharedName, { textAlign: rtl ? "right" : "left" }]}>
                      {handoff.otherContactName}
                    </Text>
                    ...
                    <Text style={[styles.sharedNote, { textAlign: rtl ? "right" : "left" }]}>
                      {copy.theirContactNote}
                    </Text>
                  </View>
                ) : (
                  <View style={styles.waitingContactCard}>
                    <Text style={[styles.waitingContactTitle, { textAlign: rtl ? "right" : "left" }]}>
                      {copy.noOtherContactTitle}
                    </Text>
                    <Text style={[styles.waitingContactBody, { textAlign: rtl ? "right" : "left" }]}>
                      {copy.noOtherContactBody}
                    </Text>
                  </View>
                )}
                ...
                {handoff.myShared ? (
                  <View style={styles.mySharedCard}>
                    <Text style={[styles.mySharedTitle, { textAlign: rtl ? "right" : "left" }]}>
                      {copy.mySharedTitle}
                    </Text>
                    <Text style={[styles.mySharedBody, { textAlign: rtl ? "right" : "left" }]}>
                      {copy.mySharedBody(handoff.myContactName ?? copy.trustedPerson)}
                    </Text>
                    <Text style={[styles.irreversibleNote, { textAlign: rtl ? "right" : "left" }]}>
                      {copy.irreversible}
                    </Text>
                  </View>
                ) : contacts.length > 0 ? (
                  <View style={styles.shareSection}>
                    <Text style={[styles.sharePickTitle, { textAlign: rtl ? "right" : "left" }]}>
                      {copy.chooseContact}
                    </Text>
                        ...
                        >
                          <View style={[styles.pickHeader, { flexDirection: rtl ? "row-reverse" : "row" }]}>
                            <View
                              style={[
                                styles.radio,
                                selectedContactId === contact.contactId ? styles.radioSelected : null,
                              ]}
                            >
                              {selectedContactId === contact.contactId ? <View style={styles.radioDot} /> : null}
                            </View>
                            <View style={styles.flex}>
                              <Text style={[styles.pickName, { textAlign: rtl ? "right" : "left" }]}>
                                {contact.displayName}
                              </Text>
                              <Text style={[styles.pickMeta, { textAlign: rtl ? "right" : "left" }]}>
                    ...
                    {confirmShare && selectedContact ? (
                      <View style={styles.confirmCard}>
                        <Text style={[styles.confirmTitle, { textAlign: rtl ? "right" : "left" }]}>
                          {copy.confirmTitle}
                        </Text>
                    ...
                    {confirmShare ? (
                      <PrimaryButton tone="quiet" disabled={sharing} onPress={() => setConfirmShare(false)}>
                        {copy.cancel}
                      </PrimaryButton>
                    ) : null}
                    <Text style={[styles.photoRule, { textAlign: rtl ? "right" : "left" }]}>
                      {copy.familyPhotoRule}
                    </Text>
                  </View>
                ) : (
              ...
              <StateCard
                rtl={rtl}
                tone="neutral"
                title={copy.handoffUnavailableTitle}
                body={copy.handoffUnavailableBody}
              />
            )
          ) : null}
              ...
              {contacts.map((contact) => (
                <View key={contact.contactId} style={styles.contactCard}>
                  <View style={[styles.contactHeader, { flexDirection: rtl ? "row-reverse" : "row" }]}>
                    ...
                    <View style={styles.flex}>
                      <Text style={[styles.contactName, { textAlign: rtl ? "right" : "left" }]}>
                        {contact.displayName}
                      </Text>
            ...
            <View style={[styles.relationships, { flexDirection: rtl ? "row-reverse" : "row" }]}>
              {relationships.map((item) => (
                <Pressable
                  ...
                  style={[styles.relationshipChip, relationship === item ? styles.relationshipChipSelected : null]}
                >
                  <Text
                    style={[styles.relationshipText, relationship === item ? styles.relationshipTextSelected : null]}
                  >
...
function trustedCopy(locale: MobileLocale) {
  ...
  return {
    ...
    handoffBody: ar
      ? "هذه الخطوة متاحة لأن القبول أصبح متبادلاً. المشاركة هنا مقصودة ومحصورة بهذا التعارف."
      : "This step is available because acceptance is mutual. Sharing here is deliberate and scoped to this introduction.",
    back: ar ? "رجوع" : "Back",
    retry: ar ? "إعادة المحاولة" : "Try again",
    loadErrorTitle: ar ? "تعذر تحميل دائرة الثقة" : "We couldn’t load your trusted contacts",
    loadErrorBody: ar
      ? "لم يتم تغيير أي شيء. تحقق من اتصالك وحاول مرة أخرى."
    ...
    handoffSectionBody: ar
      ? "ليس مطلوباً أن يشارك الطرفان في نفس الوقت. كل طرف يقرر متى يكون مستعداً لإظهار جهة اتصال موثوقة."
      : "Both people do not have to share at the same time. Each person decides when they are ready to reveal a trusted contact.",
    theirContact: ar ? "جهة الاتصال التي شاركها الطرف الآخر" : "Trusted contact shared by the other member",
    theirContactNote: ar
      ? "شارك الطرف الآخر هذه البيانات عمداً لهذا التعارف. ميثاق لم يتواصل مع هذا الشخص نيابةً عنه."
      : "The other member deliberately shared these details for this introduction. Mithaq did not contact this person on their behalf.",
    noOtherContactTitle: ar ? "لم يشارك الطرف الآخر جهة اتصال بعد" : "The other member has not shared a contact yet",
    noOtherContactBody: ar
      ? "هذا لا يمنعك من مشاركة شخصك الموثوق إذا كنت مستعداً."
      : "That does not stop you from sharing your trusted contact when you are ready.",
    mySharedTitle: ar ? "تمت مشاركة جهة اتصالك" : "Your trusted contact is shared",
    mySharedBody: (name: string) =>
      ar
        ? `يمكن للطرف الآخر الآن رؤية بيانات ${name} في هذا التعارف.`
        : `The other member can now see ${name}’s shared details in this introduction.`,
    ...
    confirmBody: (name: string, phone: string) =>
      ar
        ? `سيتمكن الطرف الآخر من رؤية اسم ${name} وصفته ورقم ${phone}. لن يرسل ميثاق رسالة لهذا الشخص.`
        : `The other member will be able to see ${name}’s name, relationship, and ${phone}. Mithaq will not message this person.`,
    irreversible: ar
      ? "بعد أن يرى الطرف الآخر هذه البيانات لا يمكن جعلها كأنها لم تُرَ. لذلك لا نوفر زر «تراجع» مضللاً."
      : "Once the other member has seen these details, they cannot be made unseen. Mithaq does not provide a misleading “undo” button.",
    ...
    familyPhotoRule: ar
      ? "إذا اخترت سابقاً أن تظهر صورتك «بعد إشراك العائلة»، فإن مشاركة جهة اتصالك هنا تعتبر بدء تلك المرحلة لصورتك أنت فقط."
      : "If your saved photo choice is “after family involvement,” sharing your own contact here starts that stage for your photo only.",
    needContact: ar
      ? "أضف شخصاً موثوقاً أدناه أولاً، ثم يمكنك اختياره لهذا التعارف."
      : "Add a trusted contact below first, then you can choose them for this introduction.",
    handoffUnavailableTitle: ar ? "التسليم العائلي غير متاح" : "Family handoff is unavailable",
    handoffUnavailableBody: ar
      ? "تتوفر المشاركة فقط داخل تعارف مقبول من الطرفين وما زال مؤهلاً وآمناً."
      : "Sharing is available only inside a mutually accepted introduction that remains eligible and safe.",
    emptyTitle: ar ? "لا يوجد شخص موثوق محفوظ بعد" : "No trusted contact saved yet",
    emptyBody: ar
      ? "يمكن أن يكون أباً أو أماً أو أخاً أو أختاً أو ولياً أو شخصاً تثق برأيه."
      : "This can be a parent, sibling, wali, guardian, relative, or another person whose judgment you trust.",
    edit: ar ? "تعديل" : "Edit",
    ...
    formBody: ar
      ? "اكتب الاسم الذي ستتعرف به عليه والطريقة الصحيحة للتواصل معه."
      : "Use the name you would introduce them by and the correct number for reaching them.",
    ...
    limitBody: ar
      ? "يمكن حفظ ثلاثة أشخاص كحد أقصى في هذه المرحلة."
      : "You can save up to three trusted contacts in this version.",
    shareTitle: ar ? "المشاركة قرار منفصل" : "Sharing is a separate decision",
    shareBody: ar
      ? "عند المشاركة نحفظ لقطة ثابتة من الاسم والصفة والرقم لذلك التعارف. تعديل الشخص المحفوظ لاحقاً لا يغير ما سبق أن شاركته."
      : "When you share, Mithaq keeps a fixed snapshot of the name, relationship, and number for that introduction. Editing the saved contact later does not silently change what you already shared.",
    ...
    shared: ar ? "تمت مشاركة جهة الاتصال لهذا التعارف." : "Trusted contact shared for this introduction.",
    phoneError: ar
      ? "اكتب رقم الهاتف بصيغة دولية تبدأ بعلامة +."
    ...
    shareError: ar
      ? "تعذر مشاركة جهة الاتصال الآن. لم يتم كشف أي بيانات جديدة."
      : "We couldn’t share that contact. No new contact details were revealed.",
    trustedPerson: ar ? "شخص موثوق" : "Trusted person",
    relationship: (value: TrustedContactRelationship) => labels[value],
  };
}
...
const styles = StyleSheet.create({
  ...
  handoffSectionTitle: { color: colors.foreground, fontSize: 17, lineHeight: 26, fontWeight: "900" },
  handoffSectionBody: { color: colors.muted, fontSize: 11, lineHeight: 19 },
  sharedContactCard: { borderRadius: radius.lg, backgroundColor: colors.primaryWash, padding: 14 },
  sharedLabel: { color: colors.primary, fontSize: 9, fontWeight: "900" },
  sharedName: { color: colors.foreground, fontSize: 17, lineHeight: 25, fontWeight: "900", marginTop: 5 },
  sharedMeta: { color: colors.muted, fontSize: 11, marginTop: 2 },
  sharedPhone: {
  ...
  sharedNote: { color: colors.muted, fontSize: 9, lineHeight: 16, marginTop: 7 },
  waitingContactCard: { borderRadius: radius.lg, backgroundColor: colors.surfaceMuted, padding: 13 },
  waitingContactTitle: { color: colors.foreground, fontSize: 12, fontWeight: "900" },
  waitingContactBody: { color: colors.muted, fontSize: 10, lineHeight: 17, marginTop: 3 },
  mySharedCard: {
    borderRadius: radius.lg,
    borderWidth: 1,
    borderColor: colors.primarySoft,
  ...
  mySharedTitle: { color: colors.primaryStrong, fontSize: 13, fontWeight: "900" },
  mySharedBody: { color: colors.foreground, fontSize: 11, lineHeight: 18, marginTop: 4 },
  irreversibleNote: { color: colors.muted, fontSize: 9, lineHeight: 16, marginTop: 7 },
  shareSection: { gap: 9 },
  sharePickTitle: { color: colors.foreground, fontSize: 12, fontWeight: "900" },
  pickList: { gap: 7 },
  pickCard: {
  ...
  pickCardSelected: { borderColor: colors.primary, backgroundColor: colors.primaryWash },
  pickHeader: { alignItems: "center", gap: 10 },
  radio: {
    width: 20,
    height: 20,
    borderRadius: 10,
  ...
  pickName: { color: colors.foreground, fontSize: 12, fontWeight: "900" },
  pickMeta: { color: colors.muted, fontSize: 9, lineHeight: 15, marginTop: 2 },
  confirmCard: { borderRadius: radius.md, backgroundColor: colors.goldSoft, padding: 12 },
  confirmTitle: { color: colors.foreground, fontSize: 12, fontWeight: "900" },
  confirmBody: { color: colors.muted, fontSize: 10, lineHeight: 17, marginTop: 3 },
  photoRule: { color: colors.muted, fontSize: 9, lineHeight: 16 },
  needContact: { color: colors.muted, fontSize: 11, lineHeight: 18 },
